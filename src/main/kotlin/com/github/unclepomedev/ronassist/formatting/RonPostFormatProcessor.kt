...
class RonPostFormatProcessor : PostFormatProcessor {
    ...
    /**
     * Post-processes the PSI element after formatting, ensuring that RON files end with a single
    ...
    /**
     * Enforces a trailing newline if the formatted range reaches the end of the file, adjusting and
     * returning the updated text range based on the modification length.
     */
    override fun processText(
        ...
        if (
            ...
            if (delta != 0) {
                val newEndOffset =
                    maxOf(rangeToReformat.startOffset, rangeToReformat.endOffset + delta)
    ...
    /**
     * Appends a trailing comma after the last element of every multiline list, map, struct or tuple
     * when [RonCodeStyleSettings.addTrailingComma] is enabled, optionally constrained to [range],
     * returning the net change in document length.
     */
    private fun addTrailingCommas(
        file: PsiFile,
        settings: CodeStyleSettings,
        ...
        val insertionOffsets =
            PsiTreeUtil.collectElements(file) {
                ...
                .mapNotNull { container ->
                    val closer =
                        container.lastChild?.takeIf { it.node.elementType in closingTokens }
                    ...
                    if (prevType == RonTypes.COMMA || prevType in openingTokens)
                        return@mapNotNull null
    ...
    /**
     * Cleans up trailing whitespaces to guarantee a single newline character at EOF, returning the
     * net change in document length (delta) to adjust formatting ranges.
     */
