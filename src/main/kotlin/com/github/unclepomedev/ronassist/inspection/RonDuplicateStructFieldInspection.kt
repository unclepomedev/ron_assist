...
class RonDuplicateStructFieldInspection : LocalInspectionTool() {
    ...
    override fun buildVisitor(
        ...
        object : PsiElementVisitor() {
            ...
            /**
             * Inspects each RonStructOrTuple in the file and reports struct entries whose field
             * name has already been seen in the same struct, leaving the first occurrence
             * untouched. Anonymous tuples are skipped because they have no named fields.
             */
                ...
                for (entry in RonPsiImplUtil.getStructEntries(element)) {
                    ...
                    if (!seen.add(name)) {
                        holder.registerProblem(
                            nameIdentifier,
                            RonInspectionBundle.message(
                                "inspection.struct.duplicate.field.problem"
                            ),
